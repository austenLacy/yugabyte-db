...
func (h *HelperContainer) CreateGoCqlClient(log logger.Logger) *gocql.ClusterConfig {
	...
	go h.GetTabletServersFuture(TserverHost, tabletServersFuture)
	...
	if tabletServersResponse.Error != nil {
		...
		// If we fail to get tservers from GetTabletServersFuture, use TserverHost
		hostNames = append(hostNames, net.JoinHostPort(TserverHost, strconv.Itoa(YcqlPort)))
	} else {
